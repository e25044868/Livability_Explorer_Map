  ...
  return (
    <div className="place-list" aria-live="polite">
      {places.map((place, index) => (
        ...
        >
          <span className={`place-rank ${place.category}`}>{place.category === 'parking' ? String(index + 1).padStart(2, '0') : place.category === 'aed' ? 'AED' : place.category === 'toilet' ? 'WC' : place.category === 'drinking_water' ? '水' : place.category === 'public_wifi' ? 'WiFi' : place.category === 'rescue_unit' ? '119' : place.category === 'police' ? '警' : place.category === 'library' ? '書' : place.category === 'public_bicycle' ? '車' : '安'}</span>
